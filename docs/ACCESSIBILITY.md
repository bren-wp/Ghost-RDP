...
- Windows High Contrast is detected through `SystemParameters.HighContrast` and the application switches its dynamic palette to Windows system colors;
...
This milestone improves the accessibility foundation but does not claim a formal WCAG or EN 301 549 conformance audit. Authentic Windows UI testing with Narrator, keyboard-only navigation, DPI scaling, and High Contrast remains part of release validation.
